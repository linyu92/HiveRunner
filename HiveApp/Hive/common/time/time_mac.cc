...
#include "common/memory/mac/scoped_cftyperef.h"
...
//  int kr = sysctl(mib, sizeof(mib), &boottime, &size, nullptr, 0);
//  DCHECK_EQ(KERN_SUCCESS, kr);
  hive::TimeDelta time_difference =
      hive::subtle::TimeNowIgnoringOverride() -
      (hive::Time::FromTimeT(boottime.tv_sec) +
       hive::TimeDelta::FromMicroseconds(boottime.tv_usec));
...
bool Time::FromExploded(bool is_local, const Exploded& exploded, Time* time) {
  hive::ScopedCFTypeRef<CFTimeZoneRef> time_zone(
      is_local
          ? CFTimeZoneCopySystem()
          : CFTimeZoneCreateWithTimeIntervalFromGMT(kCFAllocatorDefault, 0));
  hive::ScopedCFTypeRef<CFCalendarRef> gregorian(CFCalendarCreateWithIdentifier(
  ...
  hive::Time converted_time = Time(static_cast<int64_t>(microseconds));
  ...
  hive::Time::Exploded to_exploded;
  if (!is_local)
