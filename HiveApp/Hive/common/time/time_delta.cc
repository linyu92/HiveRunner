...
#include "common/memory/no_destructor.h"
...
time_t Time::ToTimeT() const {
  ...
  if (std::numeric_limits<int64_t>::max() - kTimeTToMicrosecondsOffset <= us_) {
    LOG(WARNING) << "Overflow when converting base::Time with internal " <<
                     "value " << us_ << " to time_t.";
    return std::numeric_limits<time_t>::max();
  }
...
std::ostream& operator<<(std::ostream& os, Time time) {
  ...
  return os << string_format("%04d-%02d-%02d %02d:%02d:%02d.%03d UTC",
                            exploded.year,
                            exploded.month,
...
TimeTicks TimeTicks::UnixEpoch() {
    static const hive::NoDestructor<hive::TimeTicks> epoch([]() {
