...
#include "common/memory/scoped_policy.h"
...
class ScopedTypeRef {
 public:
  ...
  explicit constexpr ScopedTypeRef(
      ...
      hive::scoped_policy::OwnershipPolicy policy = hive::scoped_policy::ASSUME)
      : object_(object) {
    if (object_ && policy == hive::scoped_policy::RETAIN)
  ...
  ScopedTypeRef& operator=(const ScopedTypeRef<T, Traits>& that) {
    reset(that.get(), hive::scoped_policy::RETAIN);
    return *this;
  }
  ...
  void reset(__unsafe_unretained T object = Traits::InvalidValue(),
             hive::scoped_policy::OwnershipPolicy policy =
                 hive::scoped_policy::ASSUME) {
    if (object && policy == hive::scoped_policy::RETAIN)
